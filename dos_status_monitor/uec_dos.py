...
def get_session(search_role) -> requests.Session:
    ...
    if search_role == "DIGITAL_REFERRAL":
        s.auth = (config.UEC_DOS_USERNAME_DIGITAL, config.UEC_DOS_PASSWORD_DIGITAL)
...
def get_services_by_service_search(
    postcode: str,
    search_distance: int,
    service_types: list,
    number_per_type: int,
    gp: str,
    search_role: str = "PATHWAYS_REFERRAL",
) -> dict:
    ...
    url = (
        f"{config.UEC_DOS_BASE_URL}/app/controllers/api/v1.0/"
...
def get_service_by_service_id(
    service_id: str, search_role: str = "PATHWAYS_REFERRAL"
) -> dict:
