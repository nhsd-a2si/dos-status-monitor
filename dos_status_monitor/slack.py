...
def send_slack_notification(
    service_name, region, capacity, old_status, service_type, changed_at, changed_by
):
        ...
        "attachments": [
            {
                ...
                "pretext": description,
                "fields": [
                    {"title": f"Previously", "value": f"{old_status}", "short": True},
                    {"title": "Region", "value": f"{region}", "short": True},
                    {"title": "Changed At", "value": f"{changed_at}", "short": True},
                    {
    ...
    logger.info(f"Sending Slack notification to " f"{config.SLACK_CHANNEL}")
...
def send_slack_status_update():
    ...
    if len(service_list) < 1:
        message_text = f"At {now}, there are no services indicating " f"low capacity."
    else:
        message_text = (
            f"At {now}, these services are indicating low "
    ...
    message = {
        ...
        "channel": slack_channel,
        "attachments": [
            {"fallback": "Capacity Status Summary", "pretext": message_text}
        ],
    }
    ...
    for service in service_list:
        ...
        field = {"title": name, "value": f"{emojis[capacity]}{capacity}", "short": True}
