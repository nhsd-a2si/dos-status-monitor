...
def has_status_changed(service_id):
    ...
    if service_snapshot:
        ...
        logger.debug(f"Previous Status: {old_status}, " f"New Status: {new_status}")
        ...
        if old_status == new_status:
            ...
            logger.info(
                f"Status has changed for {service_id} - "
...
def snapshot_service_search(probe):
    ...
    try:
        ...
        services = uec_dos.get_services_by_service_search(
            postcode, search_distance, service_types, number_per_type, gp, search_role
        )
